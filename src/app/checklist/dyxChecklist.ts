    ...
    phases: [
        ...
        },
        {
            ...
            checks: [
                ...
                check('Establish best glide ratio 9', '78 KIAS'),
                check('Select suitable landing area', 'WOS'),
                check('Select High Key / Low Key points agl', '2500ft/1500ft'),
                check('Restart - Electric fuel pump', 'ON'),
                check('Restart - FADEC switch to B (else return to auto)', 'Switch'),
                check('Restart - Engine Master switch', 'OFF->ON'),
                check('Restart - BAT master switch and ALT switch', 'check ON'),
                check('Restart - Annunciator panel and fuel levels', 'review'),
                check('Restart - FADEC circuit breakers', 'Check ON'),
                check('Restart - If low/no fuel', 'Open aux. tank transfer valve'),
                check('Restart - If propeller/engine not blocked: Starter', 'ENGAGE'),
                check('If restart not possible', 'Prepare for forced landing'),
